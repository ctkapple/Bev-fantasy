...
// The legend's dollar amounts run a green ramp by standing: the top earner is
// the deepest green, last place a pale whitish mint. Spaced by *rank position*
// rather than by dollars, so the ordering stays readable when the top two are
// within a few bucks of each other. Computed here rather than in CSS because
// the All Leagues toggle re-ranks everyone and earnings.js has to re-apply it —
// one implementation, two callers.
const RAMP_TOP = [21, 128, 61]; // #15803d
const RAMP_BOTTOM = [220, 252, 231]; // #dcfce7
export function rampColor(rank, count) {
  const t = count > 1 ? (rank - 1) / (count - 1) : 0;
  const channel = (i) =>
    Math.round(RAMP_TOP[i] + (RAMP_BOTTOM[i] - RAMP_TOP[i]) * t)
      .toString(16)
      .padStart(2, "0");
...
// Cumulative winnings only ever go up (a $0 year adds nothing, ledger entries
// are never negative), so the curve connecting them must never imply a dip —
// which is exactly the guarantee monotonePath is chosen for. See
// lib/chart-path.js, including why the zero-width segments sampleSeries emits
...
// same number of path commands. A year after the manager's run collapses onto
// their last real point, holding its slot with a repeated coordinate that
// draws nothing. Years *before* their first real point collapse onto a stub
// instead: a point half a year to the left of that first season, sitting at
// $0. Since every point between the axis start and the stub is the same
// coincident coordinate, nothing draws until the stub - then the curve rises
// from it into the real point, reading as "this is where they started"
// instead of a flat line at full height or (SB3's case, one season old) a
// bare dot with no line at all. `yearStep` is one calendar year's width in
// this view's own x-scale; when the manager's first year has nothing before
// it anywhere on the site (firstRealIdx 0), there's no prior year to stub
// against, so it falls back to the old collapse-onto-the-real-point behavior.
function sampleSeries(row, years, yearIdx, globalYears, xFor, yearStep) {
  ...
  const firstRealIdx = raw.findIndex(Boolean);
  if (firstRealIdx === -1) return null;
  const first = raw[firstRealIdx];
  const stub =
    firstRealIdx > 0 && yearStep > 0
      ? { x: +(first.x - yearStep / 2).toFixed(2), value: 0, year: first.year, real: false }
      : first;
  ...
  for (let i = 0; i < raw.length; i++) {
    if (raw[i]) {
      ...
      out.push({ ...(prev || stub), year: globalYears[i], real: false });
    }
  }
  return out;
}
...
function buildMarket(rows, years, globalYears, { buyIn, slugs }) {
  ...
  lifetime.forEach((r, i) => {
    r.rank = i + 1;
    r.valColor = rampColor(i + 1, lifetime.length);
  });
  ...
  // A league that has only ever run one season (SB3, by convention, until it
  // has a second) can't space itself against its own year count - dividing by
  // (years.length - 1) would divide by zero. Instead of falling back to dead
  // center, place that one season where it truly falls on the site-wide year
  // axis, so it lines up with the same year's tick in every other league's
  // chart and the "All Leagues" toggle doesn't have to jump it sideways.
  const gYearIdx = new Map(globalYears.map((y, i) => [y, i]));
  const globalStep = globalYears.length > 1 ? plotW / (globalYears.length - 1) : 0;
  const xFor = (i) => {
    if (years.length > 1) return PAD.l + (plotW * i) / (years.length - 1);
    const gi = gYearIdx.get(years[0]);
    return gi === undefined || globalStep === 0 ? PAD.l + plotW / 2 : PAD.l + globalStep * gi;
  };
  const yFor = (v) => PAD.t + plotH - (v / maxY) * plotH;
  const baselineY = +yFor(0).toFixed(2);
  const yearStep = years.length > 1 ? xFor(1) - xFor(0) : globalStep;

  const series = lifetime
    .map((r) => {
      const samples = sampleSeries(r, years, yearIdx, globalYears, xFor, yearStep);
