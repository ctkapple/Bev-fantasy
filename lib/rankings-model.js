...
// seat), the Climb (a cumulative-wins curve, everyone starting at 0 and
// climbing by however many games they won each season) and the Table (an
// all-time standings blotter).
//
...
// --- Climb geometry --------------------------------------------------------
// A fixed-size viewBox scaled by CSS, like the equity curve. Y is *cumulative*
// regular-season wins rather than a season's finish, so every manager starts
// flat at 0 the season before they first played and up is literally "more
// wins" — no bump-chart decoding required to see who has climbed the most.
const CHART = { W: 900, H: 380, PAD: { l: 50, r: 84, t: 20, b: 48 } };

// "Nice" win-count steps for the y-axis, picked so the chart tops out with a
// handful of gridlines whether the league is three seasons old or fifteen.
const Y_STEPS = [5, 10, 15, 20, 25, 30, 40, 50, 75, 100, 150, 200, 250, 500, 1000];

function buildClimb(rows, seasons) {
  const { W, H, PAD } = CHART;
  ...
  const xFor = (i) =>
    seasons.length < 2
      ...
      : +(plotLeft + (i * (plotRight - plotLeft)) / (seasons.length - 1)).toFixed(2);

  // First pass: run each manager's win total up season by season so the
  // y-scale can be sized off the real career-high before any point is placed.
  const raw = rows.map((row) => {
    ...
    seasons.forEach((year, i) => {
      ...
      points.push({
        ...
        wins: running,
        record: finish.ties
          ? `${finish.wins}-${finish.losses}-${finish.ties}`
          : `${finish.wins}-${finish.losses}`,
        title: row.titleYears.has(year),
      });
    });
    return { row, points, total: running };
  });

  const rawMax = Math.max(1, ...raw.map((r) => r.total));
  const step = Y_STEPS.find((s) => rawMax / s <= 5) || Math.ceil(rawMax / 5 / 500) * 500;
  const maxY = Math.ceil(rawMax / step) * step;
  const yFor = (wins) => +(plotBottom - (wins / maxY) * plotH).toFixed(2);
  const baselineY = yFor(0);

  const series = raw.map(({ row, points, total }) => {
    const plotted = points.map((p) => ({ ...p, y: yFor(p.wins) }));
  ...
  return {
    ...
    seasons,
    maxY,
    yTicks: Array.from({ length: maxY / step + 1 }, (_, i) => ({
      value: i * step,
      y: yFor(i * step),
    })),
...
function buildRafters(league, aggregate, colorFor) {
  ...
  banners.sort(
    (a, b) => BANNER_ORDER[a.kind] - BANNER_ORDER[b.kind] || Number(b.year) - Number(a.year)
  );
  return banners;
}
...
export function buildRankingsView(league, aggregate, people) {
  ...
  const rows = aggregate.standings.map((standing) => {
    ...
    for (const entry of manager.yearlyStandings || []) {
      const stats = manager.yearlyStats?.[entry.year];
      finishes[String(entry.year)] = {
        rank: entry.rank,
