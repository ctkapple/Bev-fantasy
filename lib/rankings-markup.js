...
/** The win-climb's <svg> innards: gridlines, season axis, one group per manager. */
export function climbMarkup(climb) {
  const gridlines = climb.yTicks
    .map(
      (t) => `<g class="rank-gridline ${t.value === 0 ? "is-baseline" : ""}">
      <line x1="${climb.plotLeft}" x2="${climb.plotRight}" y1="${t.y}" y2="${t.y}"></line>
      <text x="${climb.plotLeft - 14}" y="${t.y + 4}" text-anchor="end">${t.value}</text>
    </g>`
    )
    .join("");
  ...
  const series = [...climb.series]
    ...
    .map((s) => {
      ...
      const dots = s.dots
        .map(
          (p) =>
            `<circle class="rank-dot ${p.title ? "is-title" : ""}" cx="${p.x}" cy="${p.y}" r="4"><title>${esc(s.name)} — ${esc(p.record)} in ${esc(p.year)}, ${p.wins} career wins${p.title ? " · won the title" : ""}</title></circle>`
        )
        .join("");
      const label = s.end
        ? `<text class="rank-end-label" x="${s.end.x}" y="${s.end.y}">${esc(s.short)} · ${s.totalText}</text>`
        : "";
