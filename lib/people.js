import { PERSON_COLORS, LINE_COLORS, FRANCHISE_COLORS } from "../src/scripts/manager-colors.js";
...
// Manager display name -> identity color, for pages that show a manager by
// name without also resolving which league (and therefore which
// colorsForLeague slug) they're looking at. Covers every league's naming:
// solo managers and SB3's co-owned franchises via FRANCHISE_COLORS, plus the
// split individuals' own names (e.g. "Kevin Flaherty" on JRWLL/BB) that
// FRANCHISE_COLORS alone doesn't carry.
export const NAME_COLORS = { ...FRANCHISE_COLORS };
for (const person of Object.values(PEOPLE)) {
  if (PERSON_COLORS[person.id]) NAME_COLORS[person.name] = PERSON_COLORS[person.id];
}
